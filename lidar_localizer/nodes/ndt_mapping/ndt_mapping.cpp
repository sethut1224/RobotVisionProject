...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  // extract.setInputCloud(scan_ptr);
  ...
  if (_method_type == MethodType::PCL_GENERIC)
  {
    ...
    // transformation_probability = icp.getTransformationProbability();
  }
  else if (_method_type == MethodType::PCL_ANH)
  {
  ...
  std::cout << "tf_ltob: " << tf_ltob << std::endl;
...
int main(int argc, char** argv)
{
  ...
  tf_btol = Eigen::Matrix4f::Identity();
  tf_ltob = Eigen::Matrix4f::Identity();
#ifndef CUDA_FOUND
  ...
  // ros::Subscriber points_sub = nh.subscribe("points_no_ground", 100000, points_callback);
